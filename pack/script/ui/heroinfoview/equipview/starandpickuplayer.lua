...
local l_tbOtherConfig = require("script/cfg/other/config");
...
function KGC_UI_EQUIP_REINFORCE_LAYER_TYPE:EquipStar()
	...
	if self.m_equipSelected then
		...
		if nStar >= l_tbOtherConfig.nMaxEquipStar then
...
function KGC_UI_EQUIP_REINFORCE_LAYER_TYPE:UpdateRate()
	...
	local szString = nTotal .. "%+" .. 0 .. "%"
	self.m_tfRate:setText(szString)
end
...
function KGC_UI_EQUIP_REINFORCE_LAYER_TYPE:PickRecover()
