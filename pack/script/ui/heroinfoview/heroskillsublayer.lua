...
function KGC_HERO_SKILL_SUBLAYER:LoadSkills(hero)
	...
	local skillLvUpconfig = require("script/cfg/skilllevelup")

	local function fun_chooseSkill(sender,eventType)
		if eventType==ccui.TouchEventType.ended then
		...
		if bAbleUse ==true then 
			for j=1,2 do
				...
				if skillItem and skill then
					...
					Panel_button.cost = i;
					Panel_button:addTouchEventListener(fun_chooseSkill)
