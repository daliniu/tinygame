...
local TB_STRUCT_LINEUP_LAYER={
	-- config
	m_WAIT_TIME = 256;							-- 单位(ms)
	m_bUpdate = true,							-- 是否update
	m_szJsonFile = CUI_JSON_HERO_LINEUP_PATH,	-- 界面对应的json文件
	-------------------------------------------------
	m_pLayout = nil,				-- 主界面
	...
	m_nTouchHeroTimeStart = 0,		-- 初次点击英雄的时间
	m_wgtSelected = nil,			-- 选中的英雄控件
}

KGC_LINEUP_LAYER_TYPE = class("KGC_LINEUP_LAYER_TYPE", KGC_UI_BASE_LAYER, TB_STRUCT_LINEUP_LAYER)

function KGC_LINEUP_LAYER_TYPE:ctor()
end

--@function: 
function KGC_LINEUP_LAYER_TYPE:Init(tbArg)
	self:LoadScheme();
...
--@function: 
function KGC_LINEUP_LAYER_TYPE:OnInit(tbArg)
end

function KGC_LINEUP_LAYER_TYPE:OnExit()

end

--@function: 每帧更新
function KGC_LINEUP_LAYER_TYPE:Update(dt)
	-- 实现长按的逻辑
	if self.m_wgtSelected and self.m_wgtSelected:isHighlighted() then
		local nTimeEnd = os.clock();
		local bIsShowHeroInfo = (nTimeEnd - self.m_nTouchHeroTimeStart)*1000 > self.m_WAIT_TIME;
		local hero = self:GetHeroByWidget(self.m_wgtSelected);
		if bIsShowHeroInfo then
			KGC_HERO_INFO_SUBLAYER_TYPE:create(self, {hero});
			self.m_wgtSelected = nil;
		end
	else
		self.m_wgtSelected = nil;
	end
...
--@function: 加载界面
function KGC_LINEUP_LAYER_TYPE:LoadScheme()
	if not self.m_pLayout then
		cclog("[Error]界面文件错误！@KGC_LINEUP_LAYER_TYPE");
		return;
	end
...
function KGC_LINEUP_LAYER_TYPE:OnLoadScheme()
end

--@function: 加载阵上英雄
function KGC_LINEUP_LAYER_TYPE:LoadHerosOnline()
	local tbHeros = me:GetHeros();
	
	-- 上阵英雄
	local fnTouchHeroOnline = function(sender, eventType)
		self:EventTouchHero(sender, eventType);
	end
...
--@function: 加载未上阵英雄
function KGC_LINEUP_LAYER_TYPE:LoadHerosOffline()
	-- 每一行最多放四个
	local MAX_COL = 4;
...
function KGC_LINEUP_LAYER_TYPE:SetSelectArrow(widget)
...
--@function: 设置英雄为选中状态
--@pnlHero: 英雄控件
function KGC_LINEUP_LAYER_TYPE:SetHeroPanelVisible(pnlHero, bVisible)
	if not pnlHero then
		return;
	end
	local bVisible = bVisible or false;
...
--@function: 处理触摸事件
function KGC_LINEUP_LAYER_TYPE:EventTouchHero(sender, eventType)
	if eventType == ccui.TouchEventType.began then
		print("began ... ");
		self.m_nTouchHeroTimeStart = os.clock();
		self.m_wgtSelected = sender;
	elseif eventType == ccui.TouchEventType.ended then
		print("touch hero offline ... ", self.m_wgtSelected)
		-- 没有被长按代替
		if self.m_wgtSelected then
			self:TouchHero(sender);
		end
	elseif eventType == ccui.TouchEventType.moved then
		print("moved ... ", sender:isHighlighted());
...
--@function 点击英雄触发事件
function KGC_LINEUP_LAYER_TYPE:TouchHero(widget)
	if not widget then
		return;
	end

	print("TouchHero ... ", tostring(widget), widget._position);
	-- 先检测是否需要选中
...
function KGC_LINEUP_LAYER_TYPE:CheckIsSelected()
	return self.m_wgtHero and true or false;
end

--@function: 检测是否可以换位置
function KGC_LINEUP_LAYER_TYPE:CheckIsSwap(widgetSrc, widgetDst)
	local bRet = true;
...
--@widgetSrc: 第一个选中的英雄
--@widgetDst: 要互换的英雄
function KGC_LINEUP_LAYER_TYPE:SwapHero(widgetSrc, widgetDst)	
	local heroSrc = self:GetHeroByWidget(widgetSrc);
	local heroDst = self:GetHeroByWidget(widgetDst);
	if not heroSrc and not heroDst then
	...
	-- 向服务器请求
	KGC_LINEUP_LOGIC_TYPE:getInstance():ReqHeroLineup(nHeroID, nPos, {widgetSrc, widgetDst});
end

--@function: 更新一个英雄面板
function KGC_LINEUP_LAYER_TYPE:UpdateHero(widget, nSuit)
	if not widget then
		return;
	end
...
function KGC_LINEUP_LAYER_TYPE:UpdateEquips(widget, nSuit)
...
function KGC_LINEUP_LAYER_TYPE:GetHeroByWidget(widget)	
	if not widget then
...
function KGC_LINEUP_LAYER_TYPE:GetPosByWidget(widget)	
	if not widget then
		cclog("[Error]控件无效@GetHeroByWidget(widget(%s))", tostring(widget));
		return 0;
	end
...
function KGC_LINEUP_LAYER_TYPE:GetOnlineHeroPanel()	
...
function KGC_LINEUP_LAYER_TYPE:ProcessOnlineHeroPanel(wgtSrc, wgtDst)
...
function KGC_LINEUP_LAYER_TYPE:GetPos2PanelName(nPos)
...
function KGC_LINEUP_LAYER_TYPE:UpdateLineup(tbArg)
	print("UpdateLineup ................", nID)
...
function KGC_LINEUP_LAYER_TYPE:SwapHeroIDOfWidget(wgtSrc, wgtDst)
...
function KGC_LINEUP_LAYER_TYPE:SetFighetPointVisible(bVisible)
	local bVisible = bVisible or false;
	for i = 1, 3 do
		local szName = "pnl_hero" .. i;
		local pnlHero = self.m_pnlMain:getChildByName(szName);
...
function KGC_LINEUP_LAYER_TYPE:UpdateStars(svStars, nStar, szStarBg)
	print("KGC_LINEUP_LAYER_TYPE:UpdateStars", svStars, nStar, szStarBg)
