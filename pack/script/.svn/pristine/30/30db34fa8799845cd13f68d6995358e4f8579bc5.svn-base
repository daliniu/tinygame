...
require("script/ui/herolistview/hero_suit_attribute_sublayer")
...
function KGC_HERO_LIST_LAYER_TYPE:Init()
	...
	-- 初始化当前英雄默认是中间的一个
	self.m_nCurHero = self.m_REF_POS;
...
function KGC_HERO_LIST_LAYER_TYPE:TouchEventHeroBaseInfo(heroID)
	local hero =nil 

	if heroID ~=nil then 
		hero = me:GetHeroByID(heroID);
	end

	if hero==nil then 
		local tbHeros = me:GetHeros();
		hero = tbHeros[self.m_nCurHero]
	end
...
function KGC_HERO_LIST_LAYER_TYPE:LoadHeros()
	...
	for i = 1, 3 do
		...
		local hero = me:GetHeroByPos(i+3);
...
function KGC_HERO_LIST_LAYER_TYPE:UpdateHeros()
	...
	local nSelectedIndex = hero:GetPos() - 3;
	for i = 1, 3 do
		local szName = "img_herobg" .. i;
		local imgHero = self.m_pnlMain:getChildByName(szName);
		local imgSelected = imgHero:getChildByName("img_select")
...
function KGC_HERO_LIST_LAYER_TYPE:ChangeHero(nChange)
	local tbHeros = me:GetHeros();
	-- 当前正在移动或者只有一个英雄的时候，不运行移动
	if self.m_bIsMoving or #tbHeros <= 1 then
		return;
	end
...
function KGC_HERO_LIST_LAYER_TYPE:UpdatePosition(nChange)
	...
	for i, panel in pairs(tbPnlHeros) do
		...
		-- 中间一个panel的索引为2
		if nNewPos == self.m_REF_POS then
	...
	-- 计算播放哪一个动画
	cclog("nOldPos(%s)-->nChange(%s)", tostring(nOldPos), tostring(nChange));
...
--@nPos: 当前的位置
--@nMax: 位置的总数(eg: 总数是3, 当前位置如果是1, +1就变成了1)
--@nChange：变化的数值
--@modify: 2015/12/30
--[[
	oldpos --> new pos
...
function KGC_HERO_LIST_LAYER_TYPE:GetNewPosition(nPos, nMax, nChange)
	...
	cclog("GetNewPosition: oldpos(%s)-->(%s)newpos", tostring(nPos), tostring(nNew));
	return nNew;
end
